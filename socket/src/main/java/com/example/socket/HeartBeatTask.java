...
public class HeartBeatTask {
    ...
    private void heartBeatHandshake(final String heartBeatHeader) {
        ...
        if (clientHeartbeat > 0 || serverHeartbeat > 0) {
            ...
            if (clientHeartbeat > 0) {
                ...
                DLog.write("Client will send heart-beat every " + clientHeartbeat + " ms");
                scheduleClientHeartBeat();
            }
            if (serverHeartbeat > 0) {
                DLog.write("Client will listen to server heart-beat every " + serverHeartbeat + " ms");
    ...
    private void scheduleServerHeartBeatCheck() {
        if (serverHeartbeat > 0 && scheduler != null) {
            ...
            DLog.write("Scheduling server heart-beat to be checked in " + serverHeartbeat + " ms and now is '" + now + "'");
    ...
    private void checkServerHeartBeat() {
        if (serverHeartbeat > 0) {
            ...
            if (lastServerHeartBeat < boundary) {
                DLog.write("It's a sad day ;( Server didn't send heart-beat on time. Last received at '" + lastServerHeartBeat + "' and now is '" + now + "'");
                if (failedListener != null) {
                    failedListener.onServerHeartBeatFailed();
                }
            } else {
                DLog.write("We were checking and server sent heart-beat on time. So well-behaved :)");
                lastServerHeartBeat = System.currentTimeMillis();
            }
        }
    }
    ...
    private void abortServerHeartBeatCheck() {
        lastServerHeartBeat = System.currentTimeMillis();
        DLog.write("Aborted last check because server sent heart-beat on time ('" + lastServerHeartBeat + "'). So well-behaved :)");
        if (serverCheckHeartBeatTask != null) {
            serverCheckHeartBeatTask.dispose();
        }
        scheduleServerHeartBeatCheck();
    ...
    private void scheduleClientHeartBeat() {
        if (clientHeartbeat > 0 && scheduler != null) {
            DLog.write("Scheduling client heart-beat to be sent in " + clientHeartbeat + " ms");
